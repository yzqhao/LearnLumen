...
StructuredBuffer<uint> CardTileData:register(t4);
	...
	inout float3 Debug)
{
	if (all(ProbeCoordInCard < ResLevelSizeInProbes))
	{
		...
		uint2 PageCoordInCard = (ProbeCoordInCard * ProbeSpacingInRadiosityTexels) / 128;
		...
		{
			uint2 CoordInCardPage = ProbeCoordInCard * ProbeSpacingInRadiosityTexels - (PageCoordInCard * 128);
...
void CS(uint3 inGroupId : SV_GroupID,//(0~1119,0,0)
	...
	uint2 CoordInCardTile=inGroupThreadId.xy;
	...
	if (RadiosityTexel.bInsideAtlas && RadiosityTexel.bValid)
	{
		...
		FLumenCardPageData CardPage = GetLumenCardPageData(CardTile.CardPageIndex);
		FLumenCardData Card = GetLumenCardData(CardPage.CardIndex);
		uint2 CoordInCard = (CardPage.CardUVRect.xy * CardPage.ResLevelSizeInTiles + CardTile.TileCoord) * 8 + CoordInCardTile;
